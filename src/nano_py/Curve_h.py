...
def Curve_h_Leg4(R,A2,i,step,date):
...
def Curve_h_Leg5(R,A2,i,step,date):
    Angle = A2/180*math.pi
    if (R<-abs(Xb)):
        R = -R
        q1 = 0
...
def Curve_h_Leg6(R,A2,i,step,date):
    Angle = A2/180*math.pi
    if (R<-abs(Xb)):
        R = -R
...
def Curve_h_Leg3(R,A2,i,step,date):
    Angle = A2/180*math.pi
    if (R<-abs(Xb)):
        R = -R
...
def Curve_h_Leg2(R,A2,i,step,date):
    Angle = A2/180*math.pi
    if (R<-abs(Xb)):
        R = -R
        q1 = 0
...
def Curve_h_Leg1(R,A2,i,step,date):
    Angle = A2/180*math.pi
    if (R<-abs(Xb)):
        R = -R
...
def CurveLeg_h(Radius,angle,time):
    step = 8
    for i in range(0,17):
        date = bytearray(b'\x55\x55\x3B\x03\x12')
        date.extend(Uart.AddTime(65))
        Curve_h_Leg1(Radius,angle,i,step,date)
        Curve_h_Leg2(Radius,angle,i,step,date)
        Curve_h_Leg3(Radius,angle,i,step,date)
        Curve_h_Leg4(Radius,angle,i,step,date)
        Curve_h_Leg5(Radius,angle,i,step,date)
        Curve_h_Leg6(Radius,angle,i,step,date)
        Uart.SendDate(date)
        #print(date)
